...
Функция ПровестиИРаспечататьЧек(Ответ, Отказ, ФормаДокумента, РучнойРежимПечати = Ложь) Экспорт
	...
	Иначе
		...
		Если ВидФУ = Перечисления.ВидыТорговогоОборудования.ККТ Тогда
			...
			КонецЕсли;
			ПараметрыОперацииФискализацииЧека = ТорговоеОборудованиеУТВызовСервера.ПараметрыОперацииФискализацииЧекаЧекККМ(Ссылка, , , ПорядокНалогообложения);
			
			Если (ПараметрыОперацииФискализацииЧека.СистемаНалогообложения <> Перечисления.ТипыСистемНалогообложенияККТ.ЕНВД) И (ПорядокНалогообложения = "ЕНВД") Тогда
				ПараметрыОперацииФискализацииЧека.СистемаНалогообложения = Перечисления.ТипыСистемНалогообложенияККТ.ЕНВД;
			КонецЕсли;
			
			Если (ПараметрыОперацииФискализацииЧека.СистемаНалогообложения <> Перечисления.ТипыСистемНалогообложенияККТ.Патент) И (ПорядокНалогообложения = "ПСН") Тогда
				ПараметрыОперацииФискализацииЧека.СистемаНалогообложения = Перечисления.ТипыСистемНалогообложенияККТ.Патент;
			КонецЕсли;
