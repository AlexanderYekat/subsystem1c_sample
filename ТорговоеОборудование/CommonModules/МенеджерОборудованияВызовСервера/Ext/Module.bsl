...
Процедура ЗаполнитьТоварныеПозицииДляТоварногоДокумента(Ссылка, ОбщиеПараметры, КурсОплаты)
	...
	Пока Выборка.Следующий() Цикл
		...
		СтрокаЧека.КодСтраныПроисхожденияТовара = ?(ЗначениеЗаполнено(Выборка.СтранаПроисхождения), Выборка.СтранаПроисхождения.Код, "");
...
Процедура ЗаполнитьТоварнуюСтрокуЧека(ОбщиеПараметры, СтрокаПозиции, КурсОплаты)
	...
	Если ЗначениеЗаполнено(СтрокаПозиции.Серия) Тогда
		...
		СтрокаЧека.КодСтраныПроисхожденияТовара = ?(ЗначениеЗаполнено(СтрокаПозиции.Серия.СтранаПроисхождения), СтрокаПозиции.Серия.СтранаПроисхождения.Код, "");
	КонецЕсли;
		...
		Если РевизияИнтерфейса >= 3000 Тогда  // Версия стандарта 3.0 (ОФД 1.1)
			...
			Если ЧтениеXML.Имя = "OutputParameters" И ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
				Если ЧтениеXML.Прочитать() И ЧтениеXML.Имя = "Parameters" И ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда  
					...
					Если ДатаСменыККТ <> Неопределено Тогда
					...
					НеобходимаСтрочнаяЗаменаФН = ЧтениеXML.ЗначениеАтрибута("FNError");
					
					Если НеобходимаСтрочнаяЗаменаФН <> Неопределено Тогда
						Параметры.НеобходимаСтрочнаяЗаменаФН = ВРег(НеобходимаСтрочнаяЗаменаФН) = "TRUE";
					КонецЕсли;
					
					ПамятьФНПереполнена = ЧтениеXML.ЗначениеАтрибута("FNOverflow");
					
					Если ПамятьФНПереполнена <> Неопределено Тогда
						Параметры.ПамятьФНПереполнена        = ВРег(ПамятьФНПереполнена) = "TRUE";
					КонецЕсли;
					
					РесурсФНИсчерпан = ЧтениеXML.ЗначениеАтрибута("FNFail");
					
					Если РесурсФНИсчерпан <> Неопределено Тогда
						Параметры.РесурсФНИсчерпан           = ВРег(РесурсФНИсчерпан) = "TRUE";
					КонецЕсли;
...
Функция ОпределитьПризнакПредметаРасчета(Номенклатура) Экспорт
	...
	Если ТипЗнч(Номенклатура) = Тип("СправочникСсылка.Номенклатура") Тогда
		РеквизитыНоменклатуры = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Номенклатура, "АлкогольнаяПродукция, Услуга, ВидПродукцииИС");
	Иначе
		РеквизитыНоменклатуры = Новый Структура("АлкогольнаяПродукция, Услуга, ВидПродукцииИС", Ложь, Ложь, ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.ПустаяСсылка"));
	КонецЕсли;
	
	Если РеквизитыНоменклатуры.АлкогольнаяПродукция Или РеквизитыНоменклатуры.ВидПродукцииИС = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.Табак")Тогда
